...
void walltime_report(FILE * fp, int root, MPI_Comm comm) {
    ...
    if(rank != root) return;
    int i; 
    fprintf(fp, "%-16s  %10s %4s  %10s %4s  %10s %10s %s\n",
            "Comp", "Total Mean", "%", "Incre. Mean", "%", "Min", "Max", "Line");
    ...
    for(i = 0; i < CT->N; i ++) {
        ...
        fprintf(fp, "%*s%-*s  %10.2f %4.1f%%  %10.2f %4.1f%%  %10.2f %10.2f %s\n",
                level, "",  /* indents */
                16 - level, 
                name1,   /* just the last seg of name*/
                CT->AC[i].mean,
