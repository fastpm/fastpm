...
pm_nc_factor1= 4            -- Particle Mesh grid pm_nc_factor*nc per dimension in the beginning
change_pm = 0.5            -- time(scaling factor) when the pm_nc_factor is changed, range from 0 to 1
pm_nc_factor2= 4            -- Particle Mesh grid pm_nc_factor*nc per dimension after a > change_pm
...
-- FoF halo catalogue
fof= "rungroundtruth/fof"                 -- base filename
linking_factor= 0.2        -- FoF linking length= linking_factor*mean_separation

-- Dark matter particle outputs (all particles)
snapshot= "rungroundtruth/snp"       -- comment out to suppress snapshot output

-- Dark matter particle subsample
-- subsample= "rungroundtruth/sub"           -- base filename
-- subsample_factor= 1/100    -- fraction of particles to output

-- Dark matter density grid
-- coarse_grid= "rungroundtruth/grid"        -- base filename
...
measure_power = "rungroundtruth/powerspec"
