...
Na = 10000
table = np.empty((Na, 4))             #want to make dictionary like structure in .c
...
filename = "Ftable"
struct_defn = "typedef struct { double y; double F; double DF; double DDF } Ftable;\n"
struct_init = "Ftable Ft[%d]" % Na
#output to .c file
with open(filename+".c", "w+") as file:
    
    file.write("// y F(y) F'(y) F''(y)\n")
    file.write(struct_defn) #this is in .c and .h/ could include .h, but will get rid of h later.
    file.write("\n")
    file.write(struct_init)
    file.write(" = {\n\t{")
    np.savetxt(file, table, delimiter = ", ", newline = "},\n\t{")   #Yu said loop and file.write instead of this incase numpy change what this does in the future.
