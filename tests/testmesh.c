...
void fastpm_mesh_iter_free(FastPMMeshIter * iter)
{
    free(iter);
}
int fastpm_mesh_iter_next(FastPMMeshIter * iter)
{
    if(!iter->started) {
        iter->started = 1;
        return 1;
...
float ** fastpm_mesh_iter_make_table(
    FastPMMeshIter * iter,
...
double FastPMMeshKK(int ii, double dx, int nmesh)
{
    double k = 2 * M_PI / (dx * nmesh) * ii;
...
static double sinxoverx(double x) {
    if(x < 1e-5 && x > -1e-5) {
        double x2 = x * x;
...
double FastPMMeshKK3Point(int ii, double dx, int nmesh)
{
    double k = 2 * M_PI / (dx * nmesh) * ii;
    double w = k * dx;
...
double FastPMMeshKK5Point(int ii, double dx, int nmesh)
{
    double k = 2 * M_PI / (dx * nmesh) * ii;
    double w = k * dx;
...
double FastPMMeshK4Point(int ii, double dx, int nmesh)
{
