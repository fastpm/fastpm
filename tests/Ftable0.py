...
#add y=0 on end.
table0 = [0., solve_integral(I, [0.]), 0., 0.]
table = np.append(table, [table0], axis=0)

filename = "Ftable"
size_c = "size_t Fsize = %.d;\n\n" % table.shape[0]           #Note length is now Na+1 bcos of y=0, so careful! Dno if this is neatest way, but it works.
size_h = "size_t Fsize;"
array_init = "double Ftable[%d][%d]" % (table.shape[1], table.shape[0])   #swapped for .T                  #might wanna add more cols. could generalise and loop above
#output to .c file
with open(filename+".c", "w+") as file:
    
    file.write("#include <stddef.h>\n")     #need this to so that size_t type recognised. Ask Yu
