...
{
    fastpm_column_init(self, nmemb, sizeof(double), maxsize);
...
{
    fastpm_column_init(self, nmemb, sizeof(float), maxsize);
...
{
    fastpm_column_init(self, nmemb, sizeof(uint64_t), maxsize);
    self->to_double = NULL;
