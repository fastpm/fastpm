...
#include <fastpm/libfastpm.h>
        ...
        int * lua_config_get_shape_pm_nc_factor(LuaConfig * lc)
        {
            ...
            {
                luaL_eval(lc->L, "ncdm_matterlike");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
            ...
            {
                luaL_eval(lc->L, "ncdm_freestreaming");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
            ...
            int lua_config_get_growth_mode(LuaConfig * lc)
            {
                ...
                const char * str = lua_tostring(lc->L, -1);
                int val;
                if (0 == strcmp("LCDM", str)) {
                    val = FASTPM_GROWTH_MODE_LCDM;
                }
                if (0 == strcmp("ODE", str)) {
                    val = FASTPM_GROWTH_MODE_ODE;
                }
                lua_pop(lc->L, 1);
                return val;
            }
            ...
            {
                luaL_eval(lc->L, "read_linear_growth_rate");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
            ...
            {
                char * cached = (char*) lua_config_cache_get(lc, "read_linear_growth_rate");
                if(cached) return cached;
                luaL_eval(lc->L, "read_linear_growth_rate");
                const char * val = lua_tostring(lc->L, -1);
                lua_pop(lc->L, 1);
                if(val) return lua_config_cache_set(lc, "read_linear_growth_rate", _strdup(val));
                return NULL;
            }
            ...
            {
                luaL_eval(lc->L, "read_linear_growth_rate_ncdm");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
            ...
            {
                char * cached = (char*) lua_config_cache_get(lc, "read_linear_growth_rate_ncdm");
                if(cached) return cached;
                luaL_eval(lc->L, "read_linear_growth_rate_ncdm");
                const char * val = lua_tostring(lc->L, -1);
                lua_pop(lc->L, 1);
                if(val) return lua_config_cache_set(lc, "read_linear_growth_rate_ncdm", _strdup(val));
                return NULL;
            }
            ...
            {
                char * cached = (char*) lua_config_cache_get(lc, "write_rfof");
                if(cached) return cached;
                luaL_eval(lc->L, "write_rfof");
                const char * val = lua_tostring(lc->L, -1);
                lua_pop(lc->L, 1);
                if(val) return lua_config_cache_set(lc, "write_rfof", _strdup(val));
                return NULL;
            }
            int lua_config_has_rfof_kdtree_thresh(LuaConfig * lc)
            {
                luaL_eval(lc->L, "rfof_kdtree_thresh");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
            ...
            {
                luaL_eval(lc->L, "rfof_linkinglength");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
            ...
            {
                luaL_eval(lc->L, "lc_usmesh_ell_limit");
                int isnil = lua_isnil(lc->L, -1);
                lua_pop(lc->L, 1);
                return !isnil;
            }
