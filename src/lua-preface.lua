...
function parse_file(filename)
    ...
    param()
    print(dump.tostring(env))
    return dump.toscript(env)
end

-- dump.lua from http://piratery.net/dump/
...
(function()
	local t = { function() return {
		...
		success,err = dumptablecontent(table, write, level+2, dump.groupsize, ("\t"):rep(level+1)..'} end, function() return {\n', refs)
		if not success then return nil,err end
		success,err = write((([[
	} end }
	local result = {}
...
function dump.tostring(value, ignore_refs)
...
function dump.tofile(value, file)
	local cycles,refs = check_refs(value)
	if next(cycles) then
		return nil,"cycles in value",cycle_paths(cycles)
	end
...
function dump.toscript(value)
	local file = {}
	function file:write(str)
		self[#self+1] = str
		return true
	end
	local success,err,extra = dump.tofile(value, file)
	if not success then return nil,err,extra end
	return table.concat(file)
end

function dump.tofile_safe(value, filename, oldsuffix)
	...
	if not file then return nil,err end
	success,err,extra = dump.tofile(value, file)
	if not success then
		file:close()
