...
int run_fastpm(FastPMConfig * config, RunData * prr, MPI_Comm comm) {
    ...
    if(fastpm->cosmology->ncdm_linearresponse) {
        FastPMFuncK t_init[1] = {0};
        ...
        double a_transfer = 1/(1+CONF(prr->lua, linear_density_redshift));
...
write_powerspectrum(FastPMSolver * fastpm, FastPMForceEvent * event, RunData * prr) 
{
    ...
    if(CONF(prr->lua, write_powerspectrum)) {
        ...
        if(CONF(prr->lua, ncdm_linearresponse) && fastpm->ThisTask == 0) {
